...
class BombInstance(Throwable):
    ...
    def __init__(self, x, y, bomb_instances_list: list, bomb, power, damage, player=None):
        ...
        self.update_time_old = pygame.time.get_ticks()
    ...
    def update_pos(self, current_time):
        if self.throwing:
            return
        ...
        if (self.x, self.y) in cl.slide_orientation.keys():
            ...
            self.slide(self.update_time_old, False, orientation, slide_speed, top, bottom, left, right, block, top_grid, bottom_grid, left_grid, right_grid, cl)
    ...
    def update_frame(self, current_time):
        ...
        self.rect.x = self.x_pos - G.HALF_GAME_SQUARE + self.bomb["STAND"][self.bomb_frame_idx].cx - 2
        self.rect.y = self.y_pos - G.HALF_GAME_SQUARE + self.bomb["STAND"][self.bomb_frame_idx].cy - 10
            ...
            if block[1][next_point[0]][next_point[1]] > 0:
                if next_point in ois.keys():
                    ...
                    if an_obstacle.obstacle["BREAKABLE"]:
                        return 0 if block[1][next_point[0]][next_point[1]] == 2 else 1
                return 0
        ...
        if direction == "U":
            ...
            if block[0][next_point[0]][next_point[1] + 1] > 0:
                if next_point in ois.keys():
                    an_obstacle = ois[next_point]
                    if an_obstacle.obstacle["BREAKABLE"]:
                        return 0 if block[0][next_point[0]][next_point[1] + 1] == 2 else 1
                return 0
        ...
        if direction == "L":
            ...
            if block[1][next_point[0] + 1][next_point[1]] > 0:
                if next_point in ois.keys():
                    an_obstacle = ois[next_point]
                    if an_obstacle.obstacle["BREAKABLE"]:
                        return 0 if block[1][next_point[0] + 1][next_point[1]] == 2 else 1
                return 0
        ...
        if direction == "D":
            ...
            if block[0][next_point[0]][next_point[1]] > 0:
                if next_point in ois.keys():
                    an_obstacle = ois[next_point]
                    if an_obstacle.obstacle["BREAKABLE"]:
                        return 0 if block[0][next_point[0]][next_point[1]] == 2 else 1
                return 0
    ...
    def explode(self):
        ...
        if cl.me.x == self.x and cl.me.y == self.y:
            self.exEff()
        ...
        for i in distance:  # 向右爆炸延伸
            ...
            if "R" not in cl.grid_damage_orientations[point]:
                ...
                if self.bomb_timer_explode - cl.grid_damage_time[point] < cl.accumulation_time:
                    ...
                    cl.grid_damage_blood[point] = self.damage
                    cl.grid_damage_time[point] = self.bomb_timer_explode
            if cl.me.x == point[0] and cl.me.y == point[1]:
                self.exEff()
...
class BombInstanceBlack(BombInstance):

    def __init__(self, x, y, bomb_instances_list: list, bomb, power, damage, player=None):
        super().__init__(x, y, bomb_instances_list, bomb, power, damage, player)

    def exEff(self):
        # 损失1/2当前血量，减速50%
        cl = level.current_level
        cl.me.real_damage(int(cl.me.remain_blood / 2))
        cl.me.slow_for(1000, 0.5)


class BombInstanceGold(BombInstance):
    ...
    def exEff(self):
        # 补满血量，加速50%
        cl = level.current_level
        cl.me.heal(cl.me.blood)
        cl.me.slow_for(1000, -0.5)
        EffectInstance("blood_elixir_large", cl.me, True, cl.me.effects_front)
