...
class Throwable(updatable.Updatable):
    ...
    def __init__(self, x, y):
        ...
        self.throwing_time_begin = 0  # 投掷开始时刻
        self.throwing_time = pygame.time.get_ticks()  # 上次更新抛掷帧时刻
        self.direction = None  # 抛掷方向 "R" "L"
        self.points = None  # 抛物线上的点
        self.current_point = 0  # 当前在第几个点

    def get_direction(self, to_x, to_y):
        # 用于自动获取方向
        if self.x == to_x:
            return "D" if to_y > self.y else "U"
        else:
            return "R" if to_x > self.x else "L"

    def throw_to(self, to_x, to_y, direction, emit_obstacles=False):
        ...
        if not emit_obstacles and (real_x, real_y) in cl.obstacle_instances:
            if direction == "R":
            ...
            return
        self.throwing = True
        self.throwing_time_begin = pygame.time.get_ticks()
    ...
    def get_points(self, p1, p2):
        ...
        if p1[0] == p2[0]:
            ...
            # 非竖直抛掷
            # 两点式
            a = 0.002 + (0.002 - (1.0 / 280000) * abs(p2[0] - p1[0]))
    ...
    def throw(self):
        ...
        if self.current_point < len(self.points) - 1:
            # 正在投掷
            self.throwing_time = current_time
            self.current_point = min(len(self.points) - 1, (current_time - self.throwing_time_begin) // 20)
