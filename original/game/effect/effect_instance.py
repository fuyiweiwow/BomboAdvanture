...
class EffectInstance:
    ...
    def set_max_time(self, max_time):
        for i in range(len(self.effect["magics"])):
            self.effect["magics"][i]["max_time"] = max_time
    ...
    def draw(self, screen: pygame.Surface):
        ...
        for i in range(len(self.effect["magics"])):
            ...
            frame = frames[self.effect_frame_idx[i]]
            cx = self.x_pos + frame.cx + a_magic_instance["cx"]
            cy = self.y_pos + frame.cy + a_magic_instance["cy"]
            if a_magic_instance["special_flag"] < 0:
                blender.blit(frame.image, frame.alpha, screen, (int(cx), int(cy)), a_magic_instance["special_flag"])
            else:
                screen.blit(frame.image, (cx, cy), special_flags=a_magic_instance["special_flag"])
            #  BLENDMODE_NONE = 0
            #  BLEND_ADD = 1
            #  BLEND_SUB = 2
            #  BLEND_MIN = 4
...
class ReadyGo:
    ...
    def __init__(self, effect_instances):
        self.effect_instances = effect_instances
        self.state = EffectState.NORMAL
        self.ready = pygame.image.load(get_file(magic.MAGIC_IMG_ROOT + '/' + "magic0135_0_0.png")).convert_alpha()
        self.go = pygame.image.load(get_file(magic.MAGIC_IMG_ROOT + '/' + "magic0136_0_0.png")).convert_alpha()
        self.line = pygame.image.load(get_file(magic.MAGIC_IMG_ROOT + '/' + "magic0137_0_0.png")).convert_alpha()
        self.frame_idx = -1
        ...
        self.line_x_pos = (-600, -560, -520, -480, -440, -400, -360, -320, -280, -240, -200, -160, -120, -80, -40, 0, 24, 48, 72, 96, 120, 144, 168, 192, 216, 240, 264, 288, 312, 336, 360, 384, 408, 432, 456, 480, 504, 528, 552, 576, 600, 624, 648, 672, 696, 720, 720, 720, 720, 720, -600, -560, -520, -480, -440, -400, -360, -320, -280, -240, -200, -160, -120, -80, -40, 0, 24, 48, 72, 96, 120, 144, 168, 192, 216, 240, 264, 288, 312, 336, 360, 384, 408, 432, 456, 480, 504, 528, 552, 576, 600, 624, 648, 672, 696, 720, 720, 720, 720, 720, 720)
        self.line_cx = 160
        self.line_cy = 200
    ...
    def update(self):
        if self.state == EffectState.DEAD:
            ...
            return
        self.current_time = pygame.time.get_ticks()
        self.frame_idx = (self.current_time - self.init_time) // 18
        if level.current_level.score_board is not None:
            level.current_level.score_board.redraw_trigger = True
...
class DistrictAlarm:
    ...
    def __init__(self, effect_instances):
        ...
        self.district_alarm_img = pygame.image.load(get_file("res/img/magic/magic0193_0_0.png"))
    ...
    def load(self):
        ...
        self.district_alarm_imgs = pygame.Surface((x2 - x1 + G.GAME_SQUARE, y2 - y1 + G.GAME_SQUARE), pygame.SRCALPHA, 32).convert_alpha()
