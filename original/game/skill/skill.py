...
class Skill:
    ...
    def load(self):
        ...
        if isinstance(self.to, Updatable):
            self.to_x = self.to.x
            self.to_y = self.to.y
...
class RevivalCard(Skill):
    ...
    def load(self):
        ...
        self.user.rooted_for(500)
        self.user.align_xy()
        self.user.revive(int(0.1 * self.user.blood))
...
class HeiLongReverse(Skill):
    ...
    def __init__(self, user, to, skill_instances):
        ...
        self.loaded = True
        self.reversing = True
        self.reverse_effect = None

    def update(self):
        super().update()
        if self.current_time - self.skill_time_init > 4000:
            self.reverse_effect.state = EffectState.DEAD
            self.kill()
        if self.current_time - self.skill_time_init > 1000 and self.reversing:
            ...
            if self.user.remain_blood == 0:
                self.kill()
                return
            self.reverse_effect = EffectInstance("reverse", self.to, True, self.to.effects_front)
            self.effect_instances.append(self.reverse_effect)
            self.to.reverse_for(3000)
        if self.loaded:
            self.loaded = False
            self.user.rooted += 1
            self.user.align_xy()
...
class HeiLongBlackWizardPutIceTrap(Skill):
    ...
    def update(self):
        ...
        if self.current_time - self.skill_time_init > 11000:
            self.kill()
        if self.current_time - self.skill_time_init > 1000 and self.ice_trap:
            self.ice_trap = False
            self.effect_instances.append(
...
class HeiLongBlackWizardPutFireTrap(Skill):
    ...
    def update(self):
        super().update()
        if self.current_time - self.skill_time_init > 16000:
            self.kill()
        if self.current_time - self.skill_time_init > 1000 and self.fire_trap:
        ...
        if self.current_time - self.skill_time_init > 1000:
            ...
            if self.to.x == self.user_x and self.to.y == self.user_y:
                if not self.to_already_on:
                    ...
                    EffectInstance("got_fire", self.to, True, self.to.effects_front)
                    self.kill()
            else:
                self.to_already_on = False
...
class HeiLongAbyssDragonDistantFire(Skill):
    ...
    def __init__(self, user, to, skill_instances):
        self.xy_old = None  # 施法时记录玩家旧xy值
        super().__init__(user, to, skill_instances)
        self.distant_fire_animation = True
        self.distant_fire_damage = True
        self.hit = False
    ...
    def load(self):
        ...
        self.user.rooted += 1
        self.user.align_xy()
        self.xy_old = (self.to.x, self.to.y)

    def update(self):
        super().update()
        if self.current_time - self.skill_time_init > 3000:
            self.kill()
        if self.current_time - self.skill_time_init > 1600 and self.distant_fire_damage:
            self.distant_fire_damage = False
            if self.hit:
                self.to.try_damage(self.damage_blood)
        if self.current_time - self.skill_time_init > 1000 and self.distant_fire_animation:
            self.distant_fire_animation = False
            self.user.rooted -= 1
            an_effect = EffectInstance("got_burned", self.to, False, self.to.effects_front)
            an_effect.set_xy(self.xy_old[0], self.xy_old[1])  # 着火动画要在玩家的旧位置播放
            if self.to.x == self.xy_old[0] and self.to.y == self.xy_old[1]:
                self.hit = True
                self.effect_instances.append(EffectInstance("red_dots", self.to, True, self.to.effects_front))


class HeiLongAbyssDragonRecovery(Skill):

    def __init__(self, user, skill_instances):
        self.effect_recovery = None
        super().__init__(user, None, skill_instances)
        self.recovery_animation = True
        self.self_heal = [True, True, True, True, True]
    ...
    def update(self):
        super().update()
        if self.current_time - self.skill_time_init > 6000:
            self.effect_recovery.state = EffectState.DEAD
            self.kill()
        for i in range(5):  # 每秒加200血，持续5s
            if self.current_time - self.skill_time_init > 1000 * (i + 1) and self.self_heal[i]:
                self.self_heal[i] = False
                self.user.heal(200)
        if self.current_time - self.skill_time_init > 400 and self.recovery_animation:
            self.recovery_animation = False
...
class HeiLongPutFireTrap(Skill):
        ...
        if self.current_time - self.skill_time_init > 950:
            ...
            if self.to.x == self.user_x and self.to.y == self.user_y:
                if not self.to_already_on:
                    ...
                    EffectInstance("got_burned", self.to, True, self.to.effects_front)
                    self.kill()
            else:
                self.to_already_on = False


class HeiLongPutIceTrap(Skill):
    ...
    def update(self):
        ...
        if self.current_time - self.skill_time_init > 950:
            ...
            if self.to.x == self.user_x and self.to.y == self.user_y:
                if not self.to_already_on:
                    self.to.try_damage(10)
                    self.to.slow_for(1500, 0.4)
                    EffectInstance("ice_explode", self.to, True, self.to.effects_front)
                    self.kill()
            else:
                self.to_already_on = False
    ...
    def update(self):
        ...
        if not self.fire_down and self.fire_down_finish:
            ...
            for e in self.effect_fire_down:
                ...
                if e.y >= self.finish_y:
                    self.fire_down_finish = False
        if self.current_time - self.skill_time_init > 250 and self.fire_down:
            # 火焰开始下落
            self.fire_down = False
            self.user.rooted -= 1
            for e in self.effect_instances:
...
class HeiLongDizzy9x9(Skill):
    ...
    def __init__(self, user, to, skill_instances):
        ...
        self.dizzy_rooted_effect = None  # 眩晕定身特效
    ...
    def update(self):
        super().update()
        if self.current_time - self.skill_time_init > 6000:
            if self.dizzy_rooted_effect is not None:
                self.dizzy_rooted_effect.state = EffectState.DEAD
            self.kill()
        if self.current_time - self.skill_time_init > 1000 and self.dizzy_explode:
            ...
            if self.user.remain_blood == 0:
                ...
                return
            # 同行或同列眩晕
            if self.user.orientation in ("U", "D") and self.user_x == self.to.x and abs(self.user_y - self.to.y) < 5 or self.user.orientation in ("L", "R") and self.user_y == self.to.y and abs(self.user_x - self.to.x) < 5:
                self.dizzy_rooted_effect = EffectInstance("reverse", self.to, True, self.to.effects_front)
                self.effect_instances.append(self.dizzy_rooted_effect)
                self.to.rooted_for(5000)
                sound_player.play("cry")
            for x in range(-4, 5):
                for y in range(-4, 5):
        ...
        if self.current_time - self.skill_time_init > 250 and self.thunder_down:
            ...
            for x in range(-1, 2):
                for y in range(-1, 2):
                    ...
                    self.effect_instances.append(an_effect)
                    # 判断是否击中玩家
                    if self.to_xy[0] + x == self.to.x and self.to_xy[1] + y == self.to.y:
                        self.hit = True
        if self.loaded:
            self.loaded = False
            sound_player.play("launch")
        ...
        if self.current_time - self.skill_time_init > 1000:
            ...
            if self.to.x == self.user_x and self.to.y == self.user_y:
                if not self.to_already_on:
                    ...
                    EffectInstance("thunder_fixed", self.to, True, level.current_level.effects_behind)
                    self.kill()
            else:
                self.to_already_on = False


class HeiLongSkeletonKnightPutThunderTrap(HeiLongPutThunderTrap):
...
class HeiLongSkeletonKnightEnhance(Skill):
    ...
    def load(self):
        super().load()
        self.effect_instances.append(
            EffectInstance("yellow_circle", self.user, True, self.user.effects_front)
        )
        self.user.defense += 1000

    def update(self):
        super().update()
        if self.current_time - self.skill_time_init > 15000:
            self.user.defense -= 1000
            self.kill()
...
class HeiLongFloatingFire(Skill):
...
class HeiLongHellDragonScope(Skill):
    ...
    def update(self):
        ...
        if self.loaded:
            ...
            self.to_xy = (self.to.x, self.to.y)
            self.effect_instances.append(
                EffectInstance("scope", self.to, False, self.user.effects_front)
            )


class GotPoisonSmoke80x2(Skill):

    # defuff 毒物-2*80HP
    def __init__(self, user, skill_instances):
        super().__init__(user, None, skill_instances)
        self.first_hurt = True
        self.second_hurt = True

    def update(self):
        super().update()
        if self.first_hurt:
            self.first_hurt = False
            self.effect_instances.append(
                EffectInstance("poison_explode", self.user, True, self.user.effects_front)
            )
            self.user.try_damage(80)
        if self.current_time - self.skill_time_init > 8000 and self.second_hurt:
            self.second_hurt = False
            self.user.try_damage(80)
        if self.current_time - self.skill_time_init > 12000:
            self.kill()


class GotPoisonSmoke50x3(Skill):
    ...
    def update(self):
        ...
        if self.first_hurt:
            self.first_hurt = False
            self.effect_instances.append(
                EffectInstance("red_dots", self.user, True, self.user.effects_front)
            )
            self.user.try_damage(50)
            sound_player.play("cry")
        if self.current_time - self.skill_time_init > 8000 and self.second_hurt:
            self.second_hurt = False
            self.user.try_damage(50)
            sound_player.play("cry")
        if self.current_time - self.skill_time_init > 16000 and self.third_hurt:
            self.third_hurt = False
            self.user.try_damage(50)
            sound_player.play("cry")
        if self.current_time - self.skill_time_init > 20000:
            self.kill()
...
class IceSlow(Skill):
    ...
    def update(self):
        super().update()
        if self.current_time - self.skill_time_init > 9500:
            self.kill()
            return
        if self.loaded:
            self.loaded = False
            self.user.speed *= 0.7
            self.effect_instances.append(
                EffectInstance("ice_slowed", self.user, True, self.user.effects_front)
            )
            sound_player.play("cry")
