...
func build_creator_tab(vb: VBoxContainer) -> void:
	clear_container(vb)
	...
	if not _editor._hero.get("creator_mode", false):
		var btn_enable = Button.new()
		btn_enable.text = "Use Hiloan Creator"
		btn_enable.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		btn_enable.add_theme_font_size_override("font_size", 15)
		btn_enable.pressed.connect(func():
			var current_name = str(_editor._hero.get("name", "NewHiloanHero"))
			_editor._hero = HeroData.create_hiloan_creator_hero(current_name)
			_editor._dirty = true
			_editor._use_custom_tex = false
			_editor._rebuild_ui())
		inner.add_child(btn_enable)
		margin.add_child(inner)
		vb.add_child(margin)
		return

	_editor._hero["creator"] = HeroData.normalize_creator_data(_editor._hero.get("creator", {}))
	_add_creator_navigation(inner)
	match _creator_page:
		"Body":
			_add_creator_section(inner, "Body Type", "body_type", 2)
			_add_creator_color_section(inner, "Skin Tone", "skin_tone")
		"Face":
			_add_creator_section(inner, "Face Shape", "face_shape", 3)
			_add_creator_section(inner, "Ears", "ears_style", 2)
			_add_creator_section(inner, "Eyes", "eyes_style", 3)
			_add_creator_color_section(inner, "Eye Color", "eye_color")
			_add_creator_section(inner, "Eyebrows", "eyebrow_style", 2)
			if _creator_value("eyebrow_style") != "none":
				_add_creator_color_section(inner, "Brow Color", "eyebrow_color")
			_add_creator_section(inner, "Nose", "nose_style", 3)
			_add_creator_section(inner, "Mouth", "mouth_style", 3)
			_add_creator_color_section(inner, "Lip Color", "lip_color")
			_add_creator_section(inner, "Glasses", "glasses_style", 3)
			if _creator_value("glasses_style") != "none":
				_add_creator_color_section(inner, "Frame Color", "glasses_color")
		"Hair":
			_add_creator_section(inner, "Hair Style", "hair_style", 2)
			if _creator_value("hair_style") != "none":
				_add_creator_color_section(inner, "Hair Color", "hair_color")
		"Outfit":
			_add_creator_section(inner, "Clothes", "clothes_style", 2)
			if _creator_value("clothes_style") != "none":
				_add_creator_color_section(inner, "Clothes Color", "clothes_color")
			_add_creator_section(inner, "Shoes", "shoes_style", 3)
			if _creator_value("shoes_style") != "none":
				_add_creator_color_section(inner, "Shoe Color", "shoes_color")
		"Extras":
			_add_creator_section(inner, "Accessory", "accessory_style", 2)
			if _creator_value("accessory_style") != "none":
				_add_creator_color_section(inner, "Accessory Color", "accessory_color")

	margin.add_child(inner)
	vb.add_child(margin)

func _add_creator_navigation(parent: VBoxContainer) -> void:
	var row = HBoxContainer.new()
	row.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	row.add_theme_constant_override("separation", 3)
	for page in ["Body", "Face", "Hair", "Outfit", "Extras"]:
		var button = Button.new()
		button.text = page
		button.toggle_mode = true
		button.button_pressed = page == _creator_page
		button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		button.custom_minimum_size = Vector2(0, 34)
		button.add_theme_font_size_override("font_size", 11)
		button.pressed.connect(_set_creator_page.bind(page))
		row.add_child(button)
	parent.add_child(row)

func _set_creator_page(page: String) -> void:
	_creator_page = page
	build_creator_tab(_editor._tab_creator)
	_editor._set_creator_focus(page)

func _add_creator_section(parent: VBoxContainer, title: String, key: String, columns: int) -> void:
	var title_lb = Label.new()
	title_lb.text = title
	title_lb.add_theme_font_size_override("font_size", 14)
	title_lb.add_theme_color_override("font_color", Color(0.86, 0.88, 0.92))
	parent.add_child(title_lb)
	...
	var creator = _editor._hero.get("creator", {})
	var current = str(creator.get(key, ""))
	for id in HeroData.list_creator_options(key):
		var btn = Button.new()
		btn.text = HeroData.get_creator_label(str(id))
		btn.toggle_mode = true
		btn.button_pressed = str(id) == current
		btn.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		btn.custom_minimum_size = Vector2(0, 30)
		btn.add_theme_font_size_override("font_size", 12)
		btn.pressed.connect(_set_creator_option.bind(key, str(id)))
		grid.add_child(btn)

	parent.add_child(grid)

func _creator_value(key: String) -> String:
	return str(_editor._hero.get("creator", {}).get(key, ""))

func _add_creator_color_section(parent: VBoxContainer, title: String, key: String) -> void:
	var title_lb = Label.new()
	title_lb.text = title
	title_lb.add_theme_font_size_override("font_size", 14)
	title_lb.add_theme_color_override("font_color", Color(0.86, 0.88, 0.92))
	...
	for id in HeroData.list_creator_options(key):
		var button = Button.new()
		button.text = ""
		button.tooltip_text = HeroData.get_creator_label(str(id))
		button.toggle_mode = true
		button.button_pressed = str(id) == current
		button.custom_minimum_size = Vector2(40, 34)
		var normal = StyleBoxFlat.new()
		normal.bg_color = HeroData.get_creator_swatch(str(id))
		normal.border_width_left = 1
		normal.border_width_top = 1
		normal.border_width_right = 1
		normal.border_width_bottom = 1
		normal.border_color = Color(0.24, 0.26, 0.31)
		var pressed = normal.duplicate()
		pressed.border_width_left = 3
		pressed.border_width_top = 3
		pressed.border_width_right = 3
		pressed.border_width_bottom = 3
		pressed.border_color = Color(0.95, 0.82, 0.48)
		button.add_theme_stylebox_override("normal", normal)
		button.add_theme_stylebox_override("hover", pressed)
		button.add_theme_stylebox_override("pressed", pressed)
		button.pressed.connect(_set_creator_option.bind(key, str(id)))
		row.add_child(button)
	parent.add_child(row)

func _set_creator_option(key: String, value: String) -> void:
	if not _editor._hero.has("creator"):
		_editor._hero["creator"] = {}
	_editor._hero["creator"][key] = value
	_editor._dirty = true
