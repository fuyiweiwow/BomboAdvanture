...
static func build(recipe: Dictionary, overrides: Dictionary = {}) -> Dictionary:
	...
	var raw_breakable = overrides.get("breakable", recipe.get("breakable", ["elem225", "elem226", "elem227"]))
	var breakable: Array = raw_breakable.duplicate(true) if raw_breakable is Array else ["elem225", "elem226", "elem227"]
	return {
